...
# Use same distance calculation as shown in compute_distances_chunked();
...
def main():
    ...
    # How many train samples and test samples whould you like to use
    parser.add_argument('--train_samples', type=int, default=5000);
    parser.add_argument('--test_samples', type=int, default=1000);
    # PCA or not, use "--pca" to activate it
    ...
    img_train, label_train, img_test, label_test = load_cifar10(args.data_dir);
    print("Original shapes: ", img_train.shape, label_train.shape, img_test.shape, label_test.shape);

    # If we choose a subset of data, the system decides randomly
    ...
    # Optionally PCA
    if args.pca:
        print(f"Applying PCA -> {args.pca_dim} dims...");
    ...
    # Nearest centroid
    print("----------------------------------------------------");
    print("Computing nearest-centroid...");
    predict_centroid = nearest_centroid_predict(img_train, label_train, img_test);
    acc_centroid = accuracy_score(label_test, predict_centroid);
    print(f"Nearest-centroid accuracy: {acc_centroid*100:.2f}%");
    
    
    # 1-NN
    print("----------------------------------------------------");
    print("Computing 1-NN (chunk_size={})...".format(args.chunk_size));
    predict_knn1 = knn_predict(img_train, label_train, img_test, k=1, chunk_size=args.chunk_size);
    acc_knn1 = accuracy_score(label_test, predict_knn1);
    print(f"1-NN accuracy: {acc_knn1*100:.2f}%");
    

    # 3-NN
    print("----------------------------------------------------");
    print("Computing 3-NN...");
    predict_knn3 = knn_predict(img_train, label_train, img_test, k=3, chunk_size=args.chunk_size);
    acc_knn3 = accuracy_score(label_test, predict_knn3);
